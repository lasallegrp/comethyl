...
> analysis (e.g., `/path/to/my_project/`). Before running any code, set
> your working directory to the project root:
>
> ``` r
>
...
3.  After all datasets have region methylation matrices, a **shared
    ...
    sufficiently variable across both datasets.
...
7.  [`blockwiseConsensusModules()`](https://rdrr.io/pkg/WGCNA/man/blockwiseConsensusModules.html)
    identifies modules that are consistent across both sexes (Script
    09).
...
reads Bismark CpG_report files into a single BSseq object and computes
CpG totals at different coverage and per-sample cutoffs to guide filter
choice. Run once per dataset — the cytosine reports for both sexes can
live in the same directory since samples are matched via the metadata
file.
...
those columns to be silently dropped during downstream numeric
filtering.
...
![Figure 1. CpG Totals —
Females](Consensus%20Module%20Analysis/CpG_Totals_females.png)
...
retains CpGs meeting minimum coverage and per-sample requirements.
[`getRegions()`](https://lasallegrp.github.io/comethyl/reference/getRegions.md)
then clusters retained CpGs into genomic regions. These regions are the
canonical definitions reused by all other datasets. Here we keep CpGs
with at least 3 reads in at least 75% of samples (`cov = 3`,
`perSample = 0.75`).
...
![Figure 2. Unfiltered Region SD Plots —
Females](Consensus%20Module%20Analysis/Unfiltered_SD_Plots_females.png)

Figure 2. Unfiltered Region SD Plots — Females

*Figure 2. Methylation standard deviation versus region statistics for
unfiltered regions in the female dataset. Regions with low coverage tend
to have high SD driven by technical noise rather than biology.*

![Figure 3. Region Totals —
Females](Consensus%20Module%20Analysis/Region_Totals_females.png)
...
applies minimum coverage (`covMin`) and methylation SD (`methSD`)
thresholds to the raw regions.
...
Females](Consensus%20Module%20Analysis/Filtered_Region_Plots_females.png)
...
calculates region-level percent methylation from the filtered BSseq
object using the canonical reference regions from Script 02.
...
(no missing values) and sufficiently variable in both datasets to build
a good network.
...
> `--write_sd_table TRUE` flag outputs per-region SD diagnostics to
> help.
...
Summary](Consensus%20Module%20Analysis/Joint_SD_Filter_Summary.png)
...
![Figure 6. PC Variance Explained —
Females](Consensus%20Module%20Analysis/PC_Variance_Explained_females.png)
...
regresses selected PCs out of the region methylation matrix. Two
...
- **v2 (exclude protected PCs):** exclude PCs significantly correlated
  (p \< 0.05, bicor) with protected traits such as child sex or
  diagnosis. This preserves biological signal while removing technical
  confounders.
...
![Figure 8. Sample Dendrogram After Adjustment — Females
v1](Consensus%20Module%20Analysis/Sample_Dendrogram_females_v1.png)

Figure 8. Sample Dendrogram After Adjustment — Females v1

*Figure 8. Sample dendrogram based on adjusted region methylation (v1,
all PCs) for the female dataset. Samples should no longer cluster
...
analyses scale-free topology for each dataset separately across a range
of soft-thresholding powers. A single **shared consensus power** must
then be chosen — the lowest power where R² ≥ 0.8 **and** the slope is
negative in **both** females and males simultaneously. Using a shared
power is essential: it ensures the network analysis is directly
...
datasets cross R² = 0.8 with a negative slope. If the two datasets cross
at different powers, use the **higher** of the two values so that
...
![Figure 9. Soft Power Plots —
Females](Consensus%20Module%20Analysis/SoftPower_females_v1.png)
...
![Figure 10. Soft Power Plots —
Males](Consensus%20Module%20Analysis/SoftPower_males_v1.png)

Figure 10. Soft Power Plots — Males

*Figure 10. Scale-free topology fit (R²) and mean connectivity for the
male dataset. The consensus power must satisfy R² ≥ 0.8 in both datasets
— use the higher of the two crossing points if they differ.*
...
> [`getSoftPower()`](https://lasallegrp.github.io/comethyl/reference/getSoftPower.md)
> on the full region set is computationally expensive, Script 08b
> provides a faster alternative using a subsampled region set. If you
> have already run Script 08 above and are satisfied with the chosen
> power, you can skip to Script 09.
...
![Figure 11. Soft Power Benchmark Comparison — Script
08c](Consensus%20Module%20Analysis/SoftPower_benchmark_comparison.png)
...
[`blockwiseConsensusModules()`](https://rdrr.io/pkg/WGCNA/man/blockwiseConsensusModules.html)
identifies comethylation modules that are consistent across both females
and males. Two versions are run (v1 and v2 adjustment) and compared.
...
- **`deep_split`**: module detection sensitivity 0–4; 4 gives more,
  smaller modules.
- **`min_module_size`**: minimum regions per module (50 in BGW);
  increase to reduce very small modules.
- **`merge_cut_height`**: modules with eigengene correlation above this
  threshold are merged (0.2 in BGW); lower values produce fewer, broader
  modules.
...
![Figure 12. Region Dendrograms —
v1](Consensus%20Module%20Analysis/Region_Dendrograms_v1.png)
...
![Figure 13. Module Correlation Heatmap —
Females](Consensus%20Module%20Analysis/Module_Correlation_Heatmap_females.png)
...
![Figure 14. Sample ME Heatmap —
Females](Consensus%20Module%20Analysis/Sample_ME_Heatmap_females.png)
...
bicor correlation. Run separately for each dataset so sex-specific trait
...
Females](Consensus%20Module%20Analysis/ME_Trait_Heatmap_females.png)
...
Trait sets are defined in plain-text files under the
`config/presentation_sets/` directory, one trait name per line. Multiple
trait set files can be provided to generate separate heatmaps for
different groups of traits (e.g., exposures, cell types, clinical
variables).
...
![Figure 15. Cross-Dataset ME-Trait
Heatmap](Consensus%20Module%20Analysis/ME_Trait_CrossDataset_Heatmap.png)
...
![Figure 17. KEGG Enrichment — Turquoise
Module](Consensus%20Module%20Analysis/KEGG_turquoise_dotplot.png)
...
![Figure 18. Enrichr Summary — All Selected
Modules](Consensus%20Module%20Analysis/Enrichr_summary.png)
